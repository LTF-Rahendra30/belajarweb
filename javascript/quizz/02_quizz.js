...
function findProductById(products, id){
    return products.filter(produk => produk.id === id);
}

function calculateTotalPrice(products){
    return products.reduce((prdc,produk)=> prdc+produk.price,0);
}

function applyDiscount(products, discount){
    return products.map((produk)=> {
        return{
            ...produk,
            price: produk.price - (discount/100 *produk.price)
        };
    });
}
